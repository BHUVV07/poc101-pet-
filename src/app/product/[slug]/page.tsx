...
import { Heart, ShoppingBag, Plus, Minus, Star, ChevronRight, Activity, ShieldCheck, HeartHandshake } from 'lucide-react';
...
export default function ProductDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleAddToCart = () => {
    ...
    showNotification(
      'Added to Cart',
      `${quantity}x ${product.name} added to your cart successfully.`,
      'success'
    );
  };
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        ...
        <div className="w-full lg:w-1/2 space-y-4">
          <div className="aspect-square w-full rounded-lg overflow-hidden bg-surface relative shadow-sm">
            ...
            />
            {hasDiscount && (
              <span className="absolute top-4 left-4 bg-primary text-brand-bg text-xs font-semibold px-3 py-1 rounded-sm uppercase tracking-wider">
                Sale
              </span>
            )}
          </div>
        ...
        <div className="w-full lg:w-1/2 space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between border-y border-surface pb-3 pt-2">
              <div className="flex items-baseline space-x-3 text-2xl font-bold text-primary font-serif">
          ...
          {product.stock > 0 ? (
            <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
              ...
              <div className="flex items-center border border-surface rounded-full w-full sm:w-auto justify-between">
                <button
                  ...
                  className="p-3 text-text-light hover:text-text-dark cursor-pointer"
                >
              ...
              <button
                ...
                className="w-full sm:flex-1 flex items-center justify-center gap-2 rounded-full bg-primary py-3.5 px-8 text-sm font-semibold text-brand-bg hover:bg-primary/95 transition-all shadow-md cursor-pointer"
              ...
              <button
                ...
                className={`h-12 w-12 rounded-full border border-surface bg-brand-bg flex items-center justify-center cursor-pointer transition-colors ${
            ...
            <p className="text-sm font-semibold text-red-500 py-3">Out of stock. Contact support to backorder.</p>
          )}
          ...
          <div className="grid grid-cols-3 gap-4 border-t border-surface/50 pt-6 text-[10px] text-text-light">
            <div className="flex flex-col items-center text-center space-y-1.5">
              ...
              <span>Pedigree Safety Audited</span>
            </div>
            <div className="flex flex-col items-center text-center space-y-1.5">
              <ShieldCheck className="h-5 w-5 text-primary" />
              <span>Secure Manual Ledger</span>
            </div>
            <div className="flex flex-col items-center text-center space-y-1.5">
            ...
            <div className="text-sm text-text-light leading-relaxed max-w-xl">
              <table className="w-full text-left border-collapse text-xs">
                <tbody>
                  ...
                  <tr className="border-b border-surface/50"><td className="py-2.5 font-bold text-text-dark uppercase">Shipping</td><td className="py-2.5">Complimentary white-glove shipping on orders above ₹10,000</td></tr>
